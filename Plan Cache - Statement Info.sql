...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED 
 ...
 SELECT objtype
   ...
   , left(replace(replace(replace(stmt.value('(@StatementText)[1]', 'varchar(max)') , char(10), ' '), char(13), ' '), char(9), ' '), 200)                    AS SQL_Text 
...
--and (stmt.value('(@StatementOptmEarlyAbortReason)[1]', 'varchar(100)') = 'timeout'
--     or 
--     convrt.value('(@ConvertIssue)[1]', 'varchar(30)')  is not null
--      or 
--      warnrel_convrt.value('(@ConvertIssue)[1]', 'varchar(30)') is not null
-- )
